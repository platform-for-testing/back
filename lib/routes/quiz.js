...
class Quiz {
	...
	async createQuiz(ctx) {
		...
		newQuiz.questions.map((q) => {
			q.answers.map(a => a.id = uuidv4());//eslint-disable-line
			return q.id = uuidv4();//eslint-disable-line
		});
		await quizModel.create(newQuiz, (err, createdQuiz) => {
			if (err) {
	...
	async updateQuiz(ctx, id) {
		const newQuiz = ctx.request.body;
		await quizModel.update({ id }, newQuiz, (err, updatedQuiz) => {
			if (err) {
	...
	async deleteQuizById(ctx, id) {
		await quizModel.remove({ _id: id }, (err) => {
