...
class QuizResult {
	...
	static parseResults(results, quiz) {
		quiz.questions = quiz.questions.map(question => {
			const resultQuestion = results.questions.find(({ id }) => id === question.id);

			question.answers = question.answers.map(answer => {
				answer.selected = resultQuestion ? resultQuestion.answers.includes(answer.id) : false;
				return answer;
			});
			...
			console.log(question.userPoints);
	...
	async save(ctx, activationId) {
		...
		console.log(JSON.stringify(quizResult, null, 4));
