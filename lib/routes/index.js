...
module.exports = (options) => {
	...
	const upload = new Upload(db, logger, config);
	...
	router.post('/api/upload-quiz-background', async (ctx, next) => {
		await upload.uploadQuizBg(ctx);
	});
	
	router.post('/api/upload-question-image', async (ctx, next) => {
		await upload.uploadQuestionImg(ctx);
	});
	
	router.post('/api/upload-question-video', async (ctx, next) => {
		await upload.uploadQuestionVideo(ctx);
	});
};
